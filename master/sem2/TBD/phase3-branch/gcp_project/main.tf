...
resource "google_billing_account_iam_member" "tbd-terraform-billing-manager" {
  depends_on         = [google_service_account.tbd-terraform]
...
resource "google_storage_bucket" "tbd-gcp-project-state-bucket" {
  project                     = google_project.tbd_project.project_id
  name                        = "${local.project}-gcp-project-state"
  location                    = var.region
  uniform_bucket_level_access = false #tfsec:ignore:google-storage-enable-ubla
  force_destroy               = true
  lifecycle {
